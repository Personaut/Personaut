...
describe('Property 7: Boolean Variable Naming', () => {
   ...
   */
  function extractBooleanVariables(filePath: string): Array<{ name: string; line: number }> {
    const content = fs.readFileSync(filePath, 'utf-8');
    const sourceFile = ts.createSourceFile(filePath, content, ts.ScriptTarget.Latest, true);

    const booleanVars: Array<{ name: string; line: number }> = [];
  ...
  it('should use verb prefixes for boolean variables', () => {
    ...
    const violations: Array<{ file: string; variable: string; line: number }> = [];
