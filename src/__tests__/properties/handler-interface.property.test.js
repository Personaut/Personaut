...
describe('Property 17: Feature Handler Interface Implementation', () => {
     ...
     */
    function implementsIFeatureHandler(content) {
     ...
     */
    function importsIFeatureHandler(content) {
     ...
     */
    function hasHandleMethod(content) {
        // Check for handle method signature
        const handlePattern = /async\s+handle\s*\(\s*message\s*:\s*\w+,\s*webview\s*:\s*[^)]+\)\s*:\s*Promise<void>/;
    ...
    it('should have all handler files implement IFeatureHandler interface', () => {
        ...
        if (violations.length > 0) {
            throw new Error(`Handler interface violations found:\n${violations.map((v) => `  - ${v}`).join('\n')}`);
        }
    });
...
//# sourceMappingURL=handler-interface.property.test.js.map