...
describe('MCPManager', () => {
  ...
  let mockValidator: jest.Mocked<MCPValidator>;
  ...
  beforeEach(() => {
    ...
    mcpManager = new MCPManager(mockValidator, mockShowWarning);
  });
  ...
  describe('connectToServer', () => {
    ...
    it('should reject invalid MCP server configuration', async () => {
      ...
      await expect(
        mcpManager.connectToServer('test-server', 'invalid-command', [])
      ).rejects.toThrow('MCP server validation failed');

      expect(mockShowWarning).toHaveBeenCalledWith(
        'MCP Server "test-server": Command does not exist'
      );
    ...
    it('should handle connection errors', async () => {
      ...
      await expect(
        mcpManager.connectToServer('test-server', 'node', ['server.js'])
  ...
  describe('getAllTools', () => {
    it('should get tools from all connected clients', async () => {
      ...
      const { Client } = require('@modelcontextprotocol/sdk/client/index.js');
      (Client as jest.Mock).mockImplementationOnce(() => mockClient2);
  ...
  describe('setValidator', () => {
    it('should set validator instance', () => {
      const newValidator = new MCPValidator();
