...
export class BedrockProvider implements IProvider {
  ...
  private static readonly MODEL_IDS: Record<string, string> = MODEL_IDENTIFIERS.bedrock.modelIds;
  ...
  constructor(private config: ApiConfiguration) {
    ...
    if (config.awsUseProfile) {
      ...
      credentials = fromIni({ profile: config.awsProfile || 'default' });
  ...
  async chat(history: Message[], systemPrompt: string): Promise<ProviderResponse> {
    try {
      ...
      if (error.name === 'ValidationException') {
        throw new Error(
          `AWS Bedrock validation error: ${error.message}. Model ID: ${this.modelId}`
        );
        ...
        throw new Error(
          `Model not found: ${this.modelId}. Please check your AWS region (${this.config.awsRegion || DEFAULTS.awsRegion}) and ensure you have access to this model.`
        );
      } else if (
        ...
        throw new Error(
          'AWS credentials not found. Please configure one of the following in Settings:\n' +
  ...
  private formatMessages(history: Message[]): Array<{
    role: string;
    content: Array<{ type: string; text?: string; image?: any; source?: any }>;
  }> {
    ...
    for (const msg of history) {
      ...
      const content: Array<{ type: string; text?: string; image?: any; source?: any }> = [];
