...
describe('GeminiProvider', () => {
  ...
  describe('initialization', () => {
    ...
    it('should throw error when API key is missing', () => {
      ...
      expect(() => new GeminiProvider(invalidConfig)).toThrow('Gemini API Key is required');
    });
    ...
    it('should use default model when modelId is not provided', () => {
      ...
      const providerWithDefault = new GeminiProvider(configWithoutModel);
      expect(providerWithDefault).toBeInstanceOf(GeminiProvider);
    });
  });
  ...
  describe('chat method', () => {
    ...
    it('should throw error when no user message in history', async () => {
      const history: Message[] = [{ role: 'model', text: 'Hello' }];
      const systemPrompt = 'You are a helpful assistant';
    ...
    it('should filter out error messages from history', async () => {
      ...
      expect(requestBody.contents.every((c: any) => c.role !== 'error')).toBe(true);
    });
  });
});
