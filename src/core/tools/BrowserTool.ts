...
} from '../../shared/services/URLValidator';

// Dynamic import to avoid blocking extension activation
let puppeteer: typeof import('puppeteer') | null = null;

/**
 * Secure execution context for browser tool.
 */
export interface SecureBrowserToolContext {
...
export class BrowserTool implements ITool {
  name = 'browser_action';
  description =
    'Interact with a web browser. Supports launching, navigating, clicking, typing, and reading content.';
   ...
   */
  setContext(context: SecureBrowserToolContext): void {
...
`;
  ...
  private async launchOrNavigate(url?: string): Promise<string> {
    if (!url) {
      return 'Error: URL is required for launch/navigate.';
    }

    // Validate URL (Property 15: URL Validation)
    const urlValidation: URLValidationResult = this.urlValidator.validateURL(url);
    ...
    if (urlValidation.requiresConfirmation) {
      if (this.requestConfirmation) {
        const confirmed = await this.requestConfirmation(
          `Do you want to navigate to external URL: ${url}?`
        );
        if (!confirmed) {
    ...
    if (!this.browser) {
      ...
      const argsValidation: BrowserLaunchValidationResult =
        this.urlValidator.validateBrowserArgs(defaultArgs);
   ...
   */
  private getSecureBrowserArgs(): string[] {
