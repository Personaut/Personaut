 ...
 */
export const VSCODE_THEME_VARIABLES = {
  // Background colors
 ...
 */
export const ALLOWED_HARDCODED_PATTERNS = [
 ...
 */
export const SEMANTIC_COLOR_CLASS_PATTERNS = [
 ...
 */
export const STANDARD_HEADER_STYLES = {
...
export interface ButtonStyleAnalysis {
  hasRequiredClasses: boolean;
  missingClasses: string[];
  extraClasses: string[];
  buttonType: keyof typeof STANDARD_BUTTON_STYLES | 'unknown';
}

export interface HeaderStyleAnalysis {
  hasRequiredClasses: boolean;
  missingClasses: string[];
  headerType: keyof typeof STANDARD_HEADER_STYLES | 'unknown';
}

/**
 * UIStyleValidator class for validating UI styling compliance
 */
export class UIStyleValidator {
  /**
   * Check if a CSS string contains hardcoded colors that should use theme variables
   */
  containsHardcodedColors(cssContent: string): boolean {
    // First check if this is a semantic color class (which is acceptable)
    if (SEMANTIC_COLOR_CLASS_PATTERNS.some((pattern) => pattern.test(cssContent.trim()))) {
      return false;
    }
    ...
    for (const pattern of HARDCODED_COLOR_PATTERNS) {
      ...
      if (matches) {
        ...
        const disallowedMatches = matches.filter((match) => {
          return !ALLOWED_HARDCODED_PATTERNS.some((allowed) => allowed.test(match));
        });
   ...
   */
  usesThemeVariables(cssContent: string): boolean {
    ...
    for (const customVar of Object.keys(CUSTOM_THEME_VARIABLES)) {
   ...
   */
  extractHardcodedColors(cssContent: string): string[] {
    const colors: string[] = [];
    ...
    for (const pattern of HARDCODED_COLOR_PATTERNS) {
      ...
      if (matches) {
        for (const match of matches) {
          ...
          const isAllowed = ALLOWED_HARDCODED_PATTERNS.some((allowed) => allowed.test(match));
   ...
   */
  validateThemeVariableUsage(className: string): StyleValidationResult {
   ...
   */
  analyzeButtonStyle(className: string): ButtonStyleAnalysis {
    const classes = className.split(/\s+/).filter(Boolean);

    // Determine button type based on classes present
    let buttonType: keyof typeof STANDARD_BUTTON_STYLES | 'unknown' = 'unknown';
    ...
    const requiredClasses = STANDARD_BUTTON_STYLES[buttonType] as readonly string[];
    const missingClasses = requiredClasses.filter((req) => !classes.includes(req));
    const extraClasses = classes.filter(
      (cls) =>
        !(requiredClasses as readonly string[]).includes(cls) &&
        ![
          'transition-all',
          'transition-colors',
          'duration-200',
   ...
   */
  validateButtonStyle(className: string): StyleValidationResult {
   ...
   */
  analyzeHeaderStyle(className: string): HeaderStyleAnalysis {
    const classes = className.split(/\s+/).filter(Boolean);

    // Determine header type based on classes present
    let headerType: keyof typeof STANDARD_HEADER_STYLES | 'unknown' = 'unknown';
    ...
    const requiredClasses = STANDARD_HEADER_STYLES[headerType];
    const missingClasses = requiredClasses.filter((req) => !classes.includes(req));
   ...
   */
  validateHeaderStyle(className: string): StyleValidationResult {
   ...
   */
  validateThemeResponsiveness(cssContent: string): StyleValidationResult {
    const issues: StyleIssue[] = [];
    ...
    if (!usesThemeVars) {
      ...
      if (hardcodedColors.length > 0) {
        issues.push({
          ...
          suggestion:
            'Replace hardcoded colors with VS Code theme variables or custom theme variables',
        });
      }
    }
   ...
   */
  suggestThemeVariable(hardcodedColor: string): string | null {
