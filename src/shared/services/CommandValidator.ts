...
export class CommandValidator {
  ...
  private static readonly BLACKLISTED_COMMANDS: string[] = [
    'rm -rf /',
    'rm -rf /*',
    'rm -rf ~',
    'rm -rf ~/*',
   ...
   */
  detectInjection(command: string): { detected: boolean; pattern?: string } {
  ...
  sanitizeEnvironment(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
    ...
    for (const [key, value] of Object.entries(env)) {
      const isSensitive = CommandValidator.SENSITIVE_ENV_PATTERNS.some((pattern) =>
        pattern.test(key)
      );
   ...
   */
  getRateLimitStatus(): { current: number; max: number; windowMs: number; resetIn: number } {
    const now = Date.now();
    const windowStart = now - this.rateLimitConfig.windowMs;
